...
const Home = () => {
  ...
  const adjustIslandForScreenSize = () => {
    ...
    return [screenScale, screePositon, rotation];
  }

  const adjustPlaneForScreenSize = () => {
    let screenScale, screePositon;

    if (window.innerWidth < 768) {
      screenScale = [1.5, 1.5, 1.5];
      screePositon = [0, -1.5, 0];
    } else {
      screenScale = [3, 3, 3];
      screePositon = [0, -4, -4];
    }

    return [screenScale, screePositon];
  }
      ...
      >
        <Suspense fallback={<Loader />}>
          ...
          <Plane
            ...
            scale={planeScale} position={planePosition}
            rotation={[0, 20, 0]}
          />
        </Suspense>
      </Canvas>
